module movecraft::iron {

    const NAME: vector<u8> = b"Iron";
    const DESCRIPTION: vector<u8> = b"MoveCraft iron Block";
    // const URI: vector<u8> = b"data:image/svg+xml;base64,";
    const URI: vector<u8> = b"https://relive.noncegeek.com/assets/iron.jpeg";
    const BLOCK_TYPE: u64 = 1;
    const STACKABLE: bool = true;
    const MINTABLE: bool = false;

    public fun name(): vector<u8>{
        *&NAME
    }

    public fun description(): vector<u8>{
        *&DESCRIPTION
    }

    public fun uri(): vector<u8>{
        *&URI
    }

    public fun block_type():u64 {
        BLOCK_TYPE
    }

    public fun stackable():bool {
        STACKABLE
    }

    public fun mintable():bool {
        MINTABLE
    }

}