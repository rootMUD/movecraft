module movecraft::cell_0 {
    // TODO: use struct for the values.
    const NAME: vector<u8> = b"Cell #0";
    const DESCRIPTION: vector<u8> = b"MoveCraft #0 Block";
    // const URI: vector<u8> = b"data:image/svg+xml;base64,";
    const URI: vector<u8> = b"7xopmyHOuhNtH2UXomaCt8m3FK42EzJ8Fb8MuGtXU58";
    // const URI: vector<u8> = b"vKq1vpQ2gR05Hf9Nn50Ut-0j2BhtOwzBnUxxDNCuTXA";
    // const URI: vector<u8> = b"y8aRTqcRdvBmI6DwJ7_RgK22U2tcsD97vQ8Mz64IGn0";
    // const URI: vector<u8> = b"1WNPHI6RU0L91vDM9p6a7MY6AoGlO959iRPrle_0QAA";
    // const URI: vector<u8> = b"pPX-WLBK-CfLe_TdE-Bm7QURuUvFIEwok9tc_aGQjrM";
    // const URI: vector<u8> = b"IynWqymNQoSPeIjGjV0I1vhXp9mCiOzyB6F9Pbd1aoQ";
    // const URI: vector<u8> = b"uZAmafyXBL8KFeIWuLi6P3jSeRZke7oVnuAMRgup0_k";
    // const URI: vector<u8> = b"GokPo_tEy0AYT1RUrHm9fz0J29cQ0eEmYt4CT5kuq5I";

    const BLOCK_TYPE: u64 = 0;
    const STACKABLE: bool = true;
    const MINTABLE: bool = true;

    
    public fun name(): vector<u8>{
        *&NAME
    }

    public fun description(): vector<u8>{
        *&DESCRIPTION
    }

    public fun uri(): vector<u8>{
        *&URI
    }

    public fun block_type():u64 {
        BLOCK_TYPE
    }

    public fun stackable():bool {
        STACKABLE
    }

    public fun mintable():bool {
        MINTABLE
    }

}